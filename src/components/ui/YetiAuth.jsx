...
const YetiAuth = ({ emailLength = 0, focusedField = '', showPassword = false }) => {
    ...
    // Mouth paths
    const mouthSmallBGPath = "M100.2,101c-0.4,0-1.4,0-1.8,0c-2.7-0.3-5.3-1.1-8-2.5c-0.7-0.3-0.9-1.2-0.6-1.8 c0.2-0.5,0.7-0.7,1.2-0.7c0.2,0,0.5,0.1,0.6,0.2c3,1.5,5.8,2.3,8.6,2.3s5.7-0.7,8.6-2.3c0.2-0.1,0.4-0.2,0.6-0.2 c0.5,0,1,0.3,1.2,0.7c0.4,0.7,0.1,1.5-0.6,1.9c-2.6,1.4-5.3,2.2-7.9,2.5C101.7,101,100.5,101,100.2,101z";
    const mouthMediumBGPath = "M95,104.2c-4.5,0-8.2-3.7-8.2-8.2v-2c0-1.2,1-2.2,2.2-2.2h22c1.2,0,2.2,1,2.2,2.2v2 c0,4.5-3.7,8.2-8.2,8.2H95z";
    const mouthLargeBGPath = "M100 110.2c-9 0-16.2-7.3-16.2-16.2 0-2.3 1.9-4.2 4.2-4.2h24c2.3 0 4.2 1.9 4.2 4.2 0 9-7.2 16.2-16.2 16.2z";
    ...
    const bodyBGChangedPath = "M200,122h-35h-14.9V72c0-27.6-22.4-50-50-50s-50,22.4-50,50v50H35.8H0l0,91h200L200,122z";
    ...
    // Handle Email Input (Looking)
    useEffect(() => {
        if (focusedField !== 'email' && focusedField !== 'register-email') {
            // Reset face if not focused on email
            // Only reset if password is strictly NOT focused (to avoid conflict with covering eyes)
            if (!focusedField.toLowerCase().includes('password')) {
                gsap.to([eyeLRef.current, eyeRRef.current], { x: 0, y: 0, duration: 1, ease: "expo.out" });
                gsap.to(noseRef.current, { x: 0, y: 0, scaleX: 1, scaleY: 1, duration: 1, ease: "expo.out" });
                gsap.to(mouthRef.current, { x: 0, y: 0, rotation: 0, duration: 1, ease: "expo.out" });
                gsap.to(chinRef.current, { x: 0, y: 0, scaleY: 1, duration: 1, ease: "expo.out" });
                gsap.to([faceRef.current, eyebrowRef.current], { x: 0, y: 0, skewX: 0, duration: 1, ease: "expo.out" });
                gsap.to([outerEarLRef.current, outerEarRRef.current, earHairLRef.current, earHairRRef.current, hairRef.current], { x: 0, y: 0, scaleY: 1, duration: 1, ease: "expo.out" });
            }
            return;
        }

        // Calculate face position based on email length
        // This is a simplified version of the provided logic that maps length to angle
        // Max length approx 30 chars for full rotation
        const limit = 30;
        const progress = Math.min(emailLength, limit) / limit;
        const angle = (progress - 0.5) * 0.5; // -0.25 to 0.25 radians approx

        const eyeLX = Math.cos(angle) * 6;
        const eyeLY = Math.sin(angle) * 3;
        const eyeRX = Math.cos(angle) * 6;
        const eyeRY = Math.sin(angle) * 3;
        const noseX = Math.cos(angle) * 9;
        const noseY = Math.sin(angle) * 4;
        const mouthX = Math.cos(angle) * 9;
        const mouthY = Math.sin(angle) * 4;
        const mouthR = Math.cos(angle) * 2;
        const chinX = mouthX * .8;
        const chinY = mouthY * .5;
        const faceX = mouthX * .3;
        const faceY = mouthY * .4;
        const faceSkew = Math.cos(angle) * 2;

        gsap.to(eyeLRef.current, { x: eyeLX, y: eyeLY, duration: 1, ease: "expo.out" });
        gsap.to(eyeRRef.current, { x: eyeRX, y: eyeRY, duration: 1, ease: "expo.out" });
        gsap.to(noseRef.current, { x: noseX, y: noseY, rotation: mouthR, transformOrigin: "center center", duration: 1, ease: "expo.out" });
        gsap.to(mouthRef.current, { x: mouthX, y: mouthY, rotation: mouthR, transformOrigin: "center center", duration: 1, ease: "expo.out" });
        gsap.to(chinRef.current, { x: chinX, y: chinY, duration: 1, ease: "expo.out" });
        gsap.to(faceRef.current, { x: faceX, y: faceY, skewX: faceSkew, transformOrigin: "center top", duration: 1, ease: "expo.out" });

        // Mouth Shape based on '@'
        // Since we don't have the actual content here, just length, we can optionally pass content or just assume.
        // For simplicity, let's keep mouth neutral or small unless prop passed. 
        // Or if length > 0 we open mouth slightly.
        if (emailLength > 0) {
            // Morph to medium
            gsap.to(mouthBGRef.current, { attr: { d: mouthMediumBGPath }, duration: 1, ease: "expo.out" });
        } else {
            gsap.to(mouthBGRef.current, { attr: { d: mouthSmallBGPath }, duration: 1, ease: "expo.out" });
        }

    }, [emailLength, focusedField]);


    // Handle Password Focus (Cover Eyes)
    useEffect(() => {
        if (focusedField.toLowerCase().includes('password')) {
            // Cover eyes
            gsap.killTweensOf([armLRef.current, armRRef.current]);
            gsap.set([armLRef.current, armRRef.current], { visibility: "visible" });
            gsap.to(armLRef.current, { x: -93, y: 10, rotation: 0, duration: 0.45, ease: "quad.out" });
            gsap.to(armRRef.current, { x: -93, y: 10, rotation: 0, duration: 0.45, ease: "quad.out", delay: 0.1 });
            gsap.to(bodyBGRef.current, { attr: { d: bodyBGChangedPath }, duration: 0.45, ease: "quad.out" });
        } else {
            // Uncover eyes
            gsap.killTweensOf([armLRef.current, armRRef.current]);
            gsap.to(armLRef.current, { y: 220, duration: 1.35, ease: "quad.out" });
            gsap.to(armLRef.current, { rotation: 105, duration: 1.35, ease: "quad.out", delay: 0.1 });
            gsap.to(armRRef.current, { y: 220, duration: 1.35, ease: "quad.out" });
            gsap.to(armRRef.current, {
                rotation: -105, duration: 1.35, ease: "quad.out", delay: 0.1, onComplete: () => {
                    gsap.set([armLRef.current, armRRef.current], { visibility: "hidden" });
                }
            });
            gsap.to(bodyBGRef.current, { attr: { d: bodyBGNormalPath }, duration: 0.45, ease: "quad.out" });
        }
    }, [focusedField]);

    // Handle Show Password (Peek)
    useEffect(() => {
        if (showPassword) {
            gsap.to(twoFingersRef.current, { transformOrigin: "bottom left", rotation: 30, x: -9, y: -2, duration: 0.35, ease: "power2.inOut" });
        } else {
            gsap.to(twoFingersRef.current, { transformOrigin: "bottom left", rotation: 0, x: 0, y: 0, duration: 0.35, ease: "power2.inOut" });
        }
    }, [showPassword]);
    ...
    return (
        <div className="svgContainer">
            <div>
                <svg className="mySVG" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink" viewBox="0 0 200 200" ref={svgRef}>
                    <defs>
                    ...
                    <g className="body">
                        <path ref={bodyBGRef} className="bodyBG" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" fill="#FFFFFF" d="M200,158.5c0-20.2-14.8-36.5-35-36.5h-14.9V72.8c0-27.4-21.7-50.4-49.1-50.8c-28-0.5-50.9,22.1-50.9,50v50 H35.8C16,122,0,138,0,157.8L0,213h200L200,158.5z" />
                    ...
                    <path className="chin" ref={chinRef} d="M84.1 121.6c2.7 2.9 6.1 5.4 9.8 7.5l.9-4.5c2.9 2.5 6.3 4.8 10.2 6.5 0-1.9-.1-3.9-.2-5.8 3 1.2 6.2 2 9.7 2.5-.3-2.1-.7-4.1-1.2-6.1" fill="none" stroke="#3a5e77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                    <path className="face" ref={faceRef} fill="#DDF1FA" d="M134.5,46v35.5c0,21.815-15.446,39.5-34.5,39.5s-34.5-17.685-34.5-39.5V46" />
                    <path className="hair" ref={hairRef} fill="#FFFFFF" stroke="#3A5E77" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" d="M81.457,27.929 c1.755-4.084,5.51-8.262,11.253-11.77c0.979,2.565,1.883,5.14,2.712,7.723c3.162-4.265,8.626-8.27,16.272-11.235 c-0.737,3.293-1.588,6.573-2.554,9.837c4.857-2.116,11.049-3.64,18.428-4.156c-2.403,3.23-5.021,6.391-7.852,9.474" />
                    ...
                    <g className="mouth" ref={mouthRef}>
                        <path ref={mouthBGRef} className="mouthBG" fill="#617E92" d={mouthSmallBGPath} />
                        <defs>
                    ...
                    <g className="arms" clipPath="url(#armMask)">
                        <g className="armL" ref={armLRef} style={{ visibility: "hidden" }}>
                            ...
                            <g className="twoFingers" ref={twoFingersRef}>
